...
默认停在当前阶段门。用户明确授权自动完成全套流程时，复用该授权选择推荐方向并继续；仍须在进入对应图片生产前，把战略、最终商详和渠道文案分别落盘。下文与 references 中的战略、方案、最终文案及首屏确认均遵循此规则：自动模式由 Agent 记录选定方案并通过对应 QA，不反复等待用户确认。用户后续收窄范围或要求暂停时，更新当前阶段与待办；仅暂停受影响的工作。

自动完成授权只覆盖素材制作，不能补足缺失的商品事实、替代真实素材或扩大为发布授权。必须提问时指出缺口影响的具体交付，继续其他已授权且不依赖该答案的工作。
...
战略按上述阶段门确定后，读取 [pdp-copy.md](references/pdp-copy.md) 产出 2—3 套有不同说服路径的完整商详文字；按同一规则选定或合并最终方案后再开始图片工作。

最终文字按上述阶段门定稿并收到所需商品与品牌素材后，读取 [image-production.md](references/image-production.md) 制作视觉母版和逐屏图片。没有图像工具时，按 [operation-boundaries.md](references/operation-boundaries.md) 降级交付，不声称完成成图。
...
- 不读取 Cookie、API Key、浏览器配置、通讯录或无关文件；仅凭素材制作授权不自动发布商品、社媒内容、广告或外部消息。真实上架、发布和投放均需覆盖具体动作的用户明确授权；已有授权且目标、定稿与范围齐全时直接复用，具体规则见 [账号与发布](references/operation-boundaries.md#账号与发布)。
