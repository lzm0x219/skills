...
涉及缓存淘汰、依赖图、超内存数据、并发共享状态或流式 Top-K 时，读取 [scenario-checks.md](references/scenario-checks.md) 的对应分支，先确定结果语义、资源上限与失败策略，再选择结构。不能用相似的算法名称替代需求中的排序、窗口或一致性语义。

## 给出可行方案

对需要比较的实质性决策给出两个或三个不被支配的可行方案，不凑数。只有一个方案满足硬约束时直接说明；用户已委托选择时先给最终建议，只补充影响判断的取舍。每个方案说明：
...
非平凡实现先用小输入与简单可信基线做差分检查，再验证边界与操作序列；并发检查必须包含会破坏不变量的交错。需要性能证据时，按 [scenario-checks.md 的测量约定](references/scenario-checks.md#测量与停止条件) 记录可复现条件。未运行的测试列为计划，推导的复杂度与实测结果分开报告；完成适用检查后停止，不为无关场景扩大验证。
...
需要完整比较时，按以下内容组织，合并不影响决策的项目；已委托选择时先给建议，不机械铺满模板：
