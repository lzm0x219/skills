...
const livePages = [
  ...new Map([...llmsPages, ...sitemapPages].map((page) => [page.url, page])).values(),
];
...
if (args.has("--verify-links") && process.exitCode !== 1) {
  const failures = (
    await mapPool(livePages, 8, async (page) => {
      try {
