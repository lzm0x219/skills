...
- 区分最低版本、支持范围、当前编译器版本和 formatter 版本。`minimum_zig_version` 一类下界不是精确固定版本。项目支持多个版本时保留 CI 矩阵，并为每次验证选择明确编译器；改动影响版本兼容性时，至少验证最低支持版和支持范围内最新稳定版。
...
- 目标版本已明确但本地编译器不可用时，继续已授权的源码检查、版本匹配文档核查和最小修改；将编译／运行标为未执行，并列出恢复验证所需命令。工具缺失不等于源码无法审阅，也不授权更改全局工具链。
...
完成仓库必需检查，并按改动选择下列适用层级。纯文档改动运行文档检查；小范围修改从定向验证开始，涉及项目集成时再运行完整构建与测试。通过后，仅因新修改、失败或未解决疑点扩大或重复验证：
