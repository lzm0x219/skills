...
- 获取资源后按退出语义安排清理：函数内临时资源用 `defer`；成功时转移给调用者的资源用 `errdefer` 保护构造失败，由调用者在成功后释放。同一所有权不要同时安排两次清理；`errdefer` 只覆盖其所在作用域的错误退出，循环或内层作用域中逐步构造的已完成部分仍需由外层所有者管理。
- 借用值不得比其所有者、缓冲区、迭代器、容器或 C 资源存活更久。需要更长生命周期时，显式复制并记录释放者。
- 所有者仍存活也不保证借用有效：检查容器扩容、rehash、删除／移动元素、arena reset 和复用缓冲区是否使指针、切片或迭代器失效。跨变更持有身份时可保存稳定 ID，再重新获取借用；索引在删除和重排后也须重新验证。
- 为分配代码编写测试时，使用目标版本的测试 allocator 或等效的泄漏检测机制；对多阶段构造使用可控分配失败逐个覆盖失败位置，检查无泄漏、无重复释放、原对象保持约定状态。成功路径由调用者释放返回值。只跑一次成功测试不能证明 OOM 清理正确。

审阅返回值时，从返回点反查 backing storage 与清理作用域：局部数组、即将 `deinit` 的 arena、带无条件 `defer free` 的返回分配都不能作为仍有效的 owned 结果。具体语义查目标版本语言参考的 `defer`、`errdefer` 与 Lifetime and Ownership；[0.15.2 示例](https://ziglang.org/documentation/0.15.2/#errdefer) 可说明成功转移与失败清理的区别，不作为其他版本 API 的默认值。
