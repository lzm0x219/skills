...
- `mise self-update`、全局 `mise use`、全局设置、shell rc 修改、下载／安装不受信任工具、发布或部署会改变全局环境、信任范围或外部状态，未经用户明确授权不得执行。复用已有且覆盖该动作的授权。
- 用户要求配置或修复项目 CI 时，直接编辑项目内 workflow 并完成适用验证；仅规划请求保持只读。先完成已授权的配置草案、来源审查与静态检查，再处理实际缺失的信任或发布授权。
...
4. 配置或任务行为改动运行受影响的项目命令；任务定义变化且当前版本支持时，再运行 `mise tasks validate`。纯文档改动执行文档检查和仓库必需检查。通过后，仅因新修改、失败或未解决疑点扩大或重复验证。处理环境问题时，将 `mise env` 输出与预期进程环境比较，不打印敏感值。
