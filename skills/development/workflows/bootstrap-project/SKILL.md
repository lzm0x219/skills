...
每次执行为 `--report` 选择目标目录外尚不存在的新路径；保留旧报告和其他文件。适配器遇到已占用的报告路径会在初始化前拒绝，写报告时也不覆盖后到文件。

在首次写入或命令失败处停止。保留部分输出与适配器报告供诊断，保护用户拥有的文件和无关工作。

恢复 `partial` 时先核对旧报告、当前文件和已执行命令。新项目目录已有部分产物后，不再符合空目录前提；只恢复经审查的失败步骤及其尚未完成的后续步骤，或另选空目录。不得为重跑清空目录、重置用户改动或把失败的新项目直接交给 existing 适配器。全部完成门通过前仍报告 `partial`。
