...
6. `mise exec -- lefthook install --force` into the target-local `.git/hooks`;
7. `mise run ci`.
...
- Ordered Lefthook jobs: partial-stage guard, staged `zig fmt` with `stage_fixed`, staged per-file `zig ast-check`, and `mise run check`;
...
The partial-stage guard runs before any formatter. It rejects a tracked file whose index and worktree columns are both changed, preventing `stage_fixed` from expanding the commit.
